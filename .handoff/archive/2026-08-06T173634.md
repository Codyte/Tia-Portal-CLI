...
> **Corrigido em 2026-08-06.** Este handoff foi escrito 09:24 de 07-29 e o trabalho continuou
> depois dele — os passos 1 e 3 abaixo JÁ FORAM EXECUTADOS no mesmo dia. Estado real:
> `bake-lib -Apply` rodou 09:28 (`.al21` = 148 KB, 10 master copies) e `install-lib` em
> `PLC_TESTE` rodou 09:33 e **falhou** com pacote duplicado (`1.5 Diagnóstico_1`, 34 → 68 blocos,
> compile Error); fix em `Library.cs` commitado como `a0df2f7` e rebuildado, **nunca re-testado
> contra o Portal**. HEAD agora é `a0df2f7`. Detalhe em `docs/PLANO.md` → seção
> "Bake real da `.al21` + bug do `--force`". Passo 2 (UDT/tabela na `.al21`) continua não feito.
