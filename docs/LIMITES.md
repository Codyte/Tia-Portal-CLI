...
| Limite | Natureza | Saída |
|---|---|---|
| **Objeto tecnológico ainda não é criado pelo CLI** — mas a API cria. `TechnologicalInstanceDBComposition.Create(nome, tipo, Version)` e `CreateFrom(MasterCopy)` existem (doc `TOOpennessenUS/.../95672631819`, confirmados no `--sdk`); a linha anterior dizia "não tem Create" e estava **errada** — o `--sdk` mostra 15 de 18 hits por padrão e `Create` caía no corte. | Decisão do repo (pendente) | Só os tipos da lista *Overview of technology objects and versions* podem ser criados. Verbo não implementado; hoje o TO nasce na GUI e `list-motion` lê o que existe. |
