...
| **Tabela de tags da IHM não tem verbo de import no CLI** (medido 2026-08-18, F14): tela de área nova replica por `import-screen --replace`, mas as tags trocadas não existem na IHM e o `audit-screen` lista as 5. **Não é limite de API** — `Siemens.Engineering.Hmi.Tag.TagTableComposition.Import(FileInfo, ImportOptions)` existe (`tia-help.py --sdk "TagTable Import"`), só falta o verbo. | Verbo faltando | Único elo da cadeia de área nova que ainda pede GUI. `import-hmi-tags` é o par do `export-hmi-tags`. |
