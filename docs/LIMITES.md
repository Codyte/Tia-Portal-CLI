...
| **`add-call` não chama instrução do sistema nem objeto tecnológico.** Ele resolve o `--fb` na composição de blocos do PLC; `add-call --fb PID_Compact --inst <TO>` devolve `FB/FC 'PID_Compact' not found.` (medido 2026-08-21). Escrever a chamada em LAD na mão também não fecha: `<CallInfo … Version="3.0">` é recusado pelo schema (`The 'Version' attribute is not declared`) e, sem versão, o compile devolve 3 erros (`The type of the called block "PID_Compact" has changed`). | Limite do verbo (resolução por nome) | Chamar por **SCL**: `import-source` com `"<TO>"();` no corpo — compila Success 0/0, e é o que o R8 pede para lógica. |
| **Classe de evento de OB não sai de `import-source`.** Fonte SCL `ORGANIZATION_BLOCK` sempre gera `SecondaryType = ProgramCycle` (medido 2026-08-21: OB nasceu 142/ProgramCycle). Não há verbo que troque a classe nem o número. | Limite de expressão da fonte SCL | `export-block` → trocar `<SecondaryType>` (ex.: `CyclicInterrupt`), `<Number>` (30–38) e `<AutoNumber>false</AutoNumber>` → `import-block --apply`. Provado ponta a ponta com PID_Compact chamado no OB30: compile do PLC Success 0/0. O **tempo de ciclo** do OB não aparece no XML exportado nem tem verbo — é GUI. |
