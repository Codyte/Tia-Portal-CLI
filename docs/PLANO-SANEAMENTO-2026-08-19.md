...
| F6 organização | **feito** | 2ª rodada, veredito do usuário sobre os 9 achados. `1. FB Bilbiotecas` fatiada em 5 subpastas por função (1.1 Acionamento 12 · 1.2 Inversores 3 · 1.3 Instrumentação 4 · 1.4 Alarmes e Diagnóstico 11 · 1.5 Comunicação 3) e os 13 UDTs saíram da raiz para 5 pastas de domínio (2. Processo · 3. Diagnóstico · 4. Equipamento · 5. Instrumento · 6. Comunicação). Exigiu CLI novo: `import-type --folder` e `move-type`. **Duas coisas que a análise errou:** (a) `DIAG to STRING_DB` não podia ser apagado — `xref` mostra `FB DIAG MODULES ▶ NW2` usando; e movê-lo para `3.5 Barramento de Módulos` fez o `audit` reprovar em `biblioteca não depende de bloco de área`, então ele voltou para `1.4`; (b) renomear as 34 estações `SINAMICS G_xx` foi cancelado — o nome que a casa usa já está no drive object de dentro, a estação é container que ninguém olha. 49+3 steps em 2 batches, 346 s. `audit` 10/10 verde, `complete: true`, projeto salvo. |
