...
## sim
- `sim-run [--plc X] [--instance plc_1500_1] [--pc-interface PLCSIM] [--script sim.json] [--apply]  (PLC virtual do S7-PLCSIM Advanced: attach na instância que o control panel do PLCSIM Advanced já ligou, baixa o programa do projeto por Openness, roda os passos. Exige o PLCSIM CLÁSSICO FECHADO — ele toma o mesmo canal. Passos do script: ["write","tag","valor"], ["read","tag"], ["wait","ms"], ["run"], ["stop"], ["state"], ["tags","filtro"]; tag de DB vai com as aspas do Portal. Dry-run lista as instâncias registradas e as interfaces de PC do download)`
