...
namespace Tia.Core
{
    ...
    public static class Inventory
    {
        ...
        /// <summary>
        /// navindex-style outline of Program blocks: one "## folder (n)" section per folder with
        /// blocks, blocks inline as Name(TYPE num). Written to a markdown file so agents read the
        /// whole PLC layout in one cheap read (mirror of the repo's __navi__.md idea).
        /// </summary>
        public static object Tree(PlcSoftware plc, string outFile)
        {
            ...
            sb.AppendLine("# __navi__ · PLC " + plc.Name + " · Program blocks — "
                + stats[1] + " blocks in " + stats[0] + " folders");
        ...
        private static void AppendTree(PlcBlockGroup group, string path, StringBuilder body, int[] stats)
        {
            ...
            if (blocks.Any())
            {
                ...
                body.AppendLine("## " + (path.Length == 0 ? "(root)" : path) + " (" + blocks.Count + ")");
