# __navi__ · src/ (C#) — símbolos públicos por arquivo
<!-- gerado por scripts/navi-cs.ps1 · regenerar após refatorar -->

## `Tia.Cli/Program.cs` (594 linhas)
Program(16)  save-project(256)  close-project(259)  info(262)  list-devices(265)  list-blocks(268)  list-tags(271)  tree(274)  list-types(278)  find(281)  snapshot(285)  xref(288)  trace(291)  list-hmi(294)  free-memory(297)  export-block(303)  explain-block(306)  export-tags(311)  import-block(314)  import-ladder(319)  import-source(327)  create-folder(331)  delete-folder(336)  delete-block(341)  delete-type(345)  export-type(349)  import-type(352)  scaffold(356)  clone(364)  add-db-member(370)  import-tags(376)  list-library(380)  import-master-copy(383)  add-device(389)  set-address(394)  connect-subnet(400)  export-cax(405)  import-cax(408)  compile(412)  diff-block(426)  audit(430)  doctor(434)  gen-profinet(442)  standardize-tags(448)  gen-fault-ob(456)  replicate-fc(464)  gen-alarm-fc(471)  replicate-instruments(479)

## `Tia.Core/AlarmFc.cs` (649 linhas)
AlarmFcConfig(17)  AlarmFc(39)  Generate(55)  LEITURA_MUITO_ALTA(610)  LEITURA_ALTA(611)  LEITURA_BAIXA(612)  LEITURA_MUITO_BAIXA(613)  SEM_4MA(614)

## `Tia.Core/AssemblyInfo.cs` (3 linhas)


## `Tia.Core/Audit.cs` (174 linhas)
Audit(19)  TagOf(37)  CarriesTag(44)  NormalizeArea(51)  Run(61)

## `Tia.Core/BlockExplain.cs` (361 linhas)
BlockExplain(22)  Explain(24)  Statements(184)  Coil(207)  SCoil(208)  RCoil(209)  Move(210)  Contact(301)  Eq(304)  O(311)  A(313)  Call(315)

## `Tia.Core/Clone.cs` (155 linhas)
Clone(24)  Run(26)

## `Tia.Core/DbMember.cs` (161 linhas)
DbMember(21)  Add(30)  Edit(62)

## `Tia.Core/Doctor.cs` (183 linhas)
Doctor(14)  Run(23)  gen-profinet(67)  standardize-tags(84)  gen-fault-ob(93)  replicate-fc(109)  gen-alarm-fc(126)  replicate-instruments(144)

## `Tia.Core/FaultOb.cs` (323 linhas)
FaultObConfig(15)  FaultOb(35)  Module(42)  Generate(49)

## `Tia.Core/Hardware.cs` (203 linhas)
Hardware(13)  FindDevice(15)  AddDevice(48)  SetAddress(75)  ConnectSubnet(116)  CaxExport(174)  CaxImport(187)

## `Tia.Core/Hmi.cs` (43 linhas)
Hmi(10)  Targets(12)  List(23)

## `Tia.Core/InstrumentFc.cs` (639 linhas)
InstrumentFcConfig(16)  InstrumentFc(49)  Instrument(59)  AreaTask(69)  Run(77)

## `Tia.Core/Inventory.cs` (445 linhas)
Inventory(28)  Info(30)  Devices(47)  Blocks(79)  Tree(109)  TagTables(181)  Types(201)  Find(224)  Snapshot(281)  Xref(334)  Trace(382)

## `Tia.Core/LadConverter.cs` (501 linhas)
LadConverter(17)  Convert(19)  ParseAll(150)  NextUid(312)  TagAccess(314)  ConstAccess(322)  Operand(332)  NewNet(339)  Compile(341)  ToFlgNet(403)

## `Tia.Core/Library.cs` (103 linhas)
Library(15)  List(26)  ImportMasterCopy(67)

## `Tia.Core/Memory.cs` (113 linhas)
Memory(20)  FreeM(22)  X(68)  B(69)  W(70)  D(71)  BOOL(76)  WORD(77)  DWORD(78)  LWORD(79)

## `Tia.Core/Multiuser.cs` (98 linhas)
Multiuser(14)  ListServerProjects(21)

## `Tia.Core/Ops.cs` (551 linhas)
Ops(18)  FindBlock(22)  ResolveFolder(67)  ResolveTagFolder(98)  CreateFolder(130)  DeleteFolder(155)  DeleteBlock(190)  DeleteType(205)  ExportBlock(221)  ExportTagTable(236)  ExportType(246)  ImportBlock(267)  ImportTagTable(289)  ImportType(307)  ImportSource(325)  XmlRootType(379)  RequireRootType(389)  EnsureCultures(412)  DiffBlock(446)  BlocksIdentical(464)  Compile(509)

## `Tia.Core/Profinet.cs` (166 linhas)
ProfinetConfig(12)  ProfinetMapping(20)  Profinet(28)  Generate(30)  BoolAddressAllocator(143)  Next(153)  Skip(161)

## `Tia.Core/Replicate.cs` (457 linhas)
ReplicateFcConfig(15)  ReplicateFc(35)  Run(37)

## `Tia.Core/Scaffold.cs` (222 linhas)
ScaffoldManifest(22)  ScaffoldItem(36)  ScaffoldPlanItem(45)  Scaffold(58)  Plan(75)  Run(100)

## `Tia.Core/Standardize.cs` (605 linhas)
StandardizeConfig(14)  PrefixMapping(128)  TagTemplate(134)  NaturalStringComparer(141)  Compare(143)  AlarmTagComparer(170)  Compare(180)  AddressAllocator(199)  Next(212)  BYTE(227)  WORD(228)  DWORD(229)  MemoryManager(238)  AllocateBlock(247)  Standardize(272)  Run(274)

## `Tia.Core/TiaSession.cs` (190 linhas)
TiaSession(12)  Attach(23)  OpenProject(48)  CreateProject(78)  Save(113)  CloseProject(120)  AllDevices(130)  ExclusiveAccess(147)  Plcs(153)  GetPlc(165)  Dispose(185)

## `Tia.Tests/Program.cs` (424 linhas)
Program(25)

